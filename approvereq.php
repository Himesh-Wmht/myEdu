...
if($con){
  ...
  if(isset($_POST['btnUpdatej'])){
    ...
    move_uploaded_file($tname, $uploads_dir.'/'.$rfile);
	 ...
	 $ext2 = explode(".",$rfile);
	 $sreq_status= 'Confirmed';

$sql="UPDATE request SET s_id='".$s_id."', s_name='".$s_name."', s_email='".$s_email."', s_grade='".$s_grade."', s_class='".$s_class."', other='".$sreq_msg."', status='".$sreq_status."', rfile='".$rfile."' WHERE id=".$id."";
...
//Server settings
	...
	//Content
	$mail->isHTML(true);                                  
    $mail->Subject = "We Confirmed Your Request";
    $mail->Body    = "Dear $s_name, (ID: $s_id),<br> We've Recieved Your Request and we will respond soon! Your requested file is $rfile . ";
   
    $mail->send();
	 
	 echo '<script> alert("Your Confirmation Message Sent Successfully!")</script>';
...
<div class="card-body" style="margin-top: -180px;">
	...
	<br>
       ...
       <input type="text"  class="form-control" style="width:70%;color:#1DBC2F; text-align:center" placeholder="Status"   name="txtsstatus" value="Confirmed" readonly/>
